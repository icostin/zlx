...
static char const * info_src;
static unsigned int info_line;
static char const * info_func;
static char const * info_text;
...
int memalloc_debug_test (void)
{
    ...
    p = (uint32_t *) (uintptr_t) 1;
    n = 1;
    zlx_array_init(p, n);
    T(p == NULL); T(n == 0);

    r = zlx_array_alloc(&test_ma, p, n, xlarge, "bla");
    TE(r != 0, "too big alloc: r=%u p=%p n=%u", r, (void *) p, (unsigned int) n);
